...
CalculateCueWeights <- function(responseV, predictorV1, predictorV2, method){
  ...
  mod <- lm(responseV ~ predictorV1 + predictorV2)
...
for (i in 1:length(filenames)) {
    ...
    trial_ave <- ddply(raw, ~SubjectNumber+ConditionNumber+VOTlevel+F0level, summarise, perPIER = sum(Response == 1)/length(Response))
    Raw_weights <- ddply(trial_ave, ~SubjectNumber+ConditionNumber, summarise, VOT = CalculateCueWeights(perPIER, VOTlevel, F0level, method = method)[1], F0 = CalculateCueWeights(perPIER, VOTlevel, F0level, method = method)[2])
...
aveCW <- ddply(CueWeights, ~ConditionNumber+Dimensions, summarise, meanCW = mean(CW), n = length(CW), sd = sd(CW), se = sd/sqrt(n))
...
ggplot(aveCW, aes(ConditionNumber, meanCW, group = Dimensions, fill = Dimensions))+
  ...
  ggtitle('Cue Weights in different conditions(raw, using lm)')+
...
png("CueWeights_older.png", units = "in", width=10, height=8, res=500)
ggplot(ave[ave$AgeGroup == 'older',], aes(ConditionNumber, mean, fill = Dimensions))+
  geom_bar(stat = 'identity', position = position_dodge())+
  geom_errorbar(aes(ymin=mean-se, ymax = mean+se), width = 0.8, position = position_dodge(0.9), size = 0.9)+
  scale_x_discrete(labels = c('-3SNR/65dB', '0SNR/65dB', '+3SNR/65dB', 'quiet/65dB'))+
  scale_y_continuous(limits = c(-0.5,0.5))+
  labs(size = 15,
       x = 'Signal to noise levels',
       y = 'Cue Weights',
       title = 'Cue Weights in older group')+
  theme_bw()+
  theme(plot.title = element_text(hjust = 0.5), axis.title = element_text(face = 'bold', size = 18), axis.text.x = element_text(face = 'bold', color = 'black', size = 15), axis.text.y = element_text(face = 'bold', color = 'black', size = 15), legend.text = element_text(face = 'bold', color = 'black', size = 14), legend.title = element_text(face = 'bold', color = 'black', size = 14), title = element_text(face = 'bold', size = 20))
dev.off()
